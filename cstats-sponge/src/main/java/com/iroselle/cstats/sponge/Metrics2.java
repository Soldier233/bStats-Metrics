...
public class Metrics2 implements Metrics {
    ...
    public static class Factory {
         ...
         */
        public Metrics2 make() {
            return new Metrics2(plugin, logger, configDir);
        }
    }
    ...
    public static final int C_STATS_CLASS_REVISION = 2;
    ...
    private Metrics2(PluginContainer plugin, Logger logger, Path configDir) {
        this.plugin = plugin;
        this.logger = logger;
        this.configDir = configDir;
    ...
    public JsonObject getPluginData() {
        ...
        data.addProperty("metricsRevision", C_STATS_CLASS_REVISION);
    ...
    private void startSubmitting() {
        ...
        try {
            File configPath = configDir.resolve("cStats").toFile();
            configPath.mkdirs();
        ...
        builder.append("sStats metrics is present in ").append((enabled.size() + disabled.size())).append(" plugins on this server.");
        ...
        builder.append("To change the enabled/disabled state of any sStats use in a plugin, visit the Sponge config!");
        ...
        if (!configFile.exists()) {
            ...
            if (!node.getNode("configVersion").isVirtual()) {
                ...
                node.getNode("serverUuid").setComment(
                        "cStats collects some data for plugin authors like how many servers are using their plugins.\n" +
                                "To control whether this is enabled or disabled, see the Sponge configuration file.\n" +
                                "Check out https://cStats.org/ to learn more :)"
                );
